...
def downscale(pic, n):
    ...
    for i in range(rows):
        for j in range(cols):
            smaller[i, j, 0] = np.average(
                pic[i * n : i * n + n, j * n : j * n + n, 0]
            )  #    Red Channel
            smaller[i, j, 1] = np.average(
                pic[i * n : i * n + n, j * n : j * n + n, 1]
            )  # Green Channel
            smaller[i, j, 2] = np.average(
                pic[i * n : i * n + n, j * n : j * n + n, 2]
            )  # Blue Channel
    return smaller
...
def upscale_by_2(pic):
    ...
    for i in range(rows):
        ...
        for j in range(cols):
            ...
            # top-right pixel:
            larger[il, jl + 1, 0] = (
                pic[i, j, 0] + pic[i, j + 1, 0]
            ) / 2  #    Red Channel
            larger[il, jl + 1, 1] = (
                pic[i, j, 1] + pic[i, j + 1, 1]
            ) / 2  # Green Channel
            larger[il, jl + 1, 2] = (
                pic[i, j, 2] + pic[i, j + 1, 2]
            ) / 2  # Blue Channel

            # bottom-left pixel:
            larger[il + 1, jl, 0] = (
                pic[i, j, 0] + pic[i + 1, j, 0]
            ) / 2  #    Red Channel
            larger[il + 1, jl, 1] = (
                pic[i, j, 1] + pic[i + 1, j, 1]
            ) / 2  # Green Channel
            larger[il + 1, jl, 2] = (
                pic[i, j, 2] + pic[i + 1, j, 2]
            ) / 2  # Blue Channel

            # bottom-right pixel:
            larger[il + 1, jl + 1, 0] = (
                pic[i, j, 0] + pic[i + 1, j + 1, 0]
            ) / 2  #    Red Channel
            larger[il + 1, jl + 1, 1] = (
                pic[i, j, 1] + pic[i + 1, j + 1, 1]
            ) / 2  # Green Channel
            larger[il + 1, jl + 1, 2] = (
                pic[i, j, 2] + pic[i + 1, j + 1, 2]
            ) / 2  # Blue Channel
...
def get_seam(dif):
    ...
    for j in range(height):
        ...
        for i in range(width):
            ...
            if i == width - 1:
                ...
                continue
            cost[j, i] += np.amin(
                (cost[j - 1, i - 1], cost[j - 1, i], cost[j - 1, i + 1])
            )
    ...
    for j in reversed(range(height)):
        ...
        if down == width - 1:
            ...
            continue
        seam[j] = (down - 1) + np.argmin(
            (cost[j, down - 1], cost[j, down], cost[j, down + 1])
        )
    return seam
